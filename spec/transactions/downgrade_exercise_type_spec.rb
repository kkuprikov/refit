...
RSpec.describe Fitter::Transactions::ExerciseTypes::DowngradeExerciseType do
  let(:exercise_type_repo) { double(Fitter::Repositories::ExerciseTypeRepo) }
  let(:step_repo) { double(Fitter::Repositories::StepRepo) }
  
  let(:contract) { Fitter::Contracts::ExerciseTypes::DowngradeExerciseType.new(exercise_type_repo: exercise_type_repo, step_repo: step_repo) }
  
  let(:exercise_type) { Fitter::Entities::ExerciseType.new(id: 1, name: "Pull-Ups", current_step_id: 2) }
  let(:updated_exercise_type) { Fitter::Entities::ExerciseType.new(id: 1, name: "Pull-Ups", current_step_id: 1) }
  ...
  context "with valid input" do
    ...
    it "upgrades exercise type to next step" do
      ...
      expect(step_repo).to receive(:by_id).twice { step2 }
      expect(step_repo).to receive(:by_exercise_type_id_and_index) { step1 }
